...
projects.forEach(project => {
    project.addEventListener('click', () => {
        ...
        const details = project.getAttribute('data-details');
        const link = project.getAttribute('data-link');
        const image = project.querySelector('img'); // Get the image element
        ...
        // Add image to the overlay if it exists
        const overlayContent = overlay.querySelector('.overlay-content');
        let overlayImage = overlayContent.querySelector('img');
        if (!overlayImage) {
            // Create image element if it doesn't exist in the overlay
            overlayImage = document.createElement('img');
            overlayContent.insertBefore(overlayImage, overlayTitle); // Add it above the title
        }

        if (image) {
            overlayImage.src = image.src;
            overlayImage.alt = image.alt || 'Project Image'; // Use alt from project or fallback
